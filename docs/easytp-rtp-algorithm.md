...
EasyTP 是我近期开发的一个 PaperMC 传送插件，其中的 `/rtp` 命令负责把玩家送到一个随机且安全的位置。这个命令的功能定义只有一句话，但它在服务器上的实际表现对实现方案高度敏感。

LeafRTP 项目发布的基准测试[1]显示，在相同的测试框架、世界与客户端配置下，不同随机传送插件的主线程 MSPT 第 99 百分位从 $4\ \mathrm{ms}$ 跨越至 $4534\ \mathrm{ms}$，相差约 1134 倍；而除 EssentialsX 之外（其 `/tpr` 为请求式命令，数值反映请求—接受往返），其余七个插件的吞吐量落在 $1.7$–$20$ 次/秒，跨度约 12 倍。即**吞吐量差一个数量级，卡顿差三个数量级**。这说明性能差异的来源不是吞吐设计，而是单次搜索代价的分布形态。
...
我的早期实现由取点与判定两阶段构成，两者被合并在同一重试循环内。下列代码是该结构的**示意性重构**，用于说明控制流，并非当时的源码原文：
...
这里有一处需要修正：$p$ 的实际取值比 LeafRTP 统计所暗示的更小。LeafRTP 给出的 $35\%$–$65\%$ 是按"海洋、岩浆、虚空"三类**地形**统计的不安全比例；本实现默认启用 `rtp.overworld-surface-only`，要求落点露天，因而还会排除洞穴、峡谷掩体与树冠之下的位置。在真实服务端上对一个 2000–5000 环带内的候选列调用生产实现 `findSafeSpotSync`，64 列中有 6 列返回了安全落点，即 $\hat{p} = 6/64 \approx 0.094$（二项标准误约 $\pm 0.036$；测法与限制见第 10 节）。代入式 (3) 可得表 1。
...
| 0.094（实测） | 10.7 | 41.2 % | 15.4 % | 71.2 |

$p = 0.094$ 时，二十次尝试仍然失败的概率是 15.4%，比按 $p = 0.35$ 估计的高出约 550 倍；要把失败率压到 $10^{-3}$ 需要约 71 次尝试。**露天约束把拒绝采样的尾部显著拉长了**，这解释了为什么"搜索很久然后失败"是一个反复出现的现象，而不是偶发。
...
上述四类现象——面密度偏差、单次代价非平稳、尾部无界、相邻重试之间无记忆——均可追溯至同一个结构，即三项职责被压在同一个控制流内，见表 2。
...
权重决定的是**选环概率**，而各环带的面积相差数倍，因此权重同时决定了落点的面积密度。把两者放在一起看，才能读出默认配置的实际含义：

| 环 | 半径范围 | 面积 / $10^6$ 格² | 权重 | 单位面积入选概率（归一化到 `near`） |
|---|---|---|---|---|
| `near` | 2000 – 3500 | 25.92 | 20 | 1.000 |
| `mid` | 3500 – 5000 | 40.06 | 60 | 1.941 |
| `far` | 5000 – 7500 | 98.17 | 20 | 0.264 |

一个落点落在环 $j$ 的概率是 $w_j / \sum_k w_k$，而该环的面积为 $A_j$，故面积密度正比于 $w_j / A_j$。按此计算，默认配置**并不是**面积均匀的：`mid` 的面密度接近 `near` 的两倍，`far` 只有 `near` 的 26%。其实际效果是六成落点集中在 3500–5000 一带，而最外的环带占两成权重、面积却最大，因而成为最稀疏的一段。

这里存在一处容易写反的关系需要点明：**要使面积密度处处相等，权重应与面积成正比，而不是成反比**。按面积比例配置即得 $15.8 : 24.4 : 59.8$，取整为 $16 : 24 : 60$。两种取向都合理——"集中在中环"与"全域均匀"是两种不同的产品选择——但它们不能混同：权重不是与面积无关的调参旋钮，改权重直接改变落点的空间分布。式 (8) 保证的是**单个环带内部**密度恒定，跨环带的密度由这张表决定。
...
三间隙定理的适用范围也需要交代，否则上文的结论会被过度外推。该定理约束的是**固定半径圆周上**的角度序列，而实际采样点的半径随索引单调增大，两者并不等同；把定理的结论接到"二维点间距有下界"上，中间还缺一步论证。这一步由式 (6) 的导数提供：$\mathrm{d}r/\mathrm{d}i = s^2/(2\pi r)$，在 $r = 2000$、$s = 16$ 处约 $0.02$ 格/索引，比最近邻尺度（实测约 $14$ 格）小三个数量级。也就是说，在任意一对点的间距所允许的索引跨度内半径几乎不变，序列在局部近似落在同一个圆上，角向次序基本上就是二维次序。真正可能彼此靠近的一对点，必然是角向差接近 $2\pi$ 整数倍的一对，也就是索引差接近 Fibonacci 数的一对——而这恰好是三间隙定理给出定量下界的地方。

需要说明这是**论证而非定理的直接推论**：定理本身不覆盖半径递增的情形，上面的推导只解释了角向结构为何会传递到二维间距，可检验的落点仍是数值结果本身。（图 3(e) 的二维最近邻分布与图 3(f) 的固定圆周角度间隙分布是两个不同的统计量，并列给出不意味着二者可以互相替代。）
...
截断环带上可能存在放射状条纹，这一担心值得单独检验，因为它会直接抵消极向规则性的收益。检验对象必须是生成器的实际输出——即整数坐标下的整个索引周期——而不是图 3(c) 那个散点图：三个散点子图为了可直接对比采用相同样本量（4000 点），而 4000 点是从 $N_{\text{ring}} = 257708$ 个索引中随机抽取的，等价于按索引做 Bernoulli 抽稀。抽稀会先摧毁角向结构，剩下的主要是抽样噪声，因此图 3(c) 的结块程度反而接近均匀随机（方位向计数 $\sigma/\mu = 0.136$，Poisson 参考值 $0.123$）。按整周期实测：

| 统计量 | 黄金角螺旋 | 面积反变换随机 | Poisson 参考 |
|---|---|---|---|
| 方位向计数 $\sigma/\mu$（60 扇区） | 0.0008 | 0.0158 | 0.0153 |
| 方位向计数 $\sigma/\mu$（360 扇区） | 0.0031 | 0.0378 | 0.0374 |
| 100 格径向薄层内 360 扇区计数 $\sigma/\mu$ | 0.023 – 0.046 | 0.172 – 0.234 | 0.174 – 0.255 |
| 单个扇区内最大径向空隙 / 格 | 2.94 | 13.16 | — |

方位向计数的离散度比 Poisson 涨落小一个数量级，任一扇区内的最大径向空隙为 $2.94$ 格，而随机方案为 $13.16$ 格。**结论与"放射状条纹"相反：螺旋在方位向比随机采样更规则，不存在系统偏大的方位向空隙。** 这与式 (8) 并不矛盾——式 (8) 只约束径向密度，方位向的规则性是式 (10) 额外带来的。
...
**图 3** 三种一维到二维采样方案的对比（环带 2000–5000 格，$s = 16$）。(a)–(c) 为各 4000 点的等样本量散点抽样，用于目视对比；(d)–(f) 的统计量取各方案的完整样本，其中 (f) 的角度序列长度为 $N = 987$。
...
另一个自然的方向是把落点的高度一并纳入记忆：单元目前只回答"这里存在安全落点"，若同时记下该落点的 $y$，重复访问同一单元时可以直接复用，不必重扫列。这个方向可行，但收益比看上去小，原因在第 10.3 节：`ChunkSnapshot` 加列扫描合计 $0.253\ \mathrm{ms}$，只占一次磁盘区块获取的 $1.8\%$，占一次地形生成的 $1.5 \times 10^{-5}$。缓存 $y$ 省下的正是那 $1.8\%$，而重复候选真正的开销是它仍然需要的那次区块获取——缓存无法替它免掉。

语义上还有一处不对称需要注意。`SAFE` 是一个**粗糙的**断言——"这个 $32 \times 32$ 的单元内存在安全落点"——它对地形的局部改动不敏感，单元内换一个落点仍然成立。而"落点的 $y$ 值"是**精细的**断言：同一列上放置或破坏一个方块就可能使其失效。把两者放进同一张记忆表，就必须为精细断言单独设计失效条件，否则一次搭桥、一处挖掘就会把玩家送进实心方块。相比之下，按时间窗口对已提交单元去重不引入新的失效语义，这也是它更适合作为第一步的原因。
...
修正后的曲线在 tick 2 停止生成、tick 7 停止校验（模拟设定的目标容量是 20，这里是 14，故停止时刻随之提前），此后完全静止；原实现则以每 tick 2 次的速度持续生成与校验，60 个 tick 内做了 120 次区块加载，而净产出始终为零。**空闲服务器上原实现浪费的区块加载量由校验速率决定，与目标容量无关**——把 `base-size` 调小并不能缓解它。实测曲线见图 5(a)。
...
这一节与算法主线关系最弱，只保留对本设计有约束力的部分：流水线各阶段本就分布在不同线程上（表 7），而区域化调度把"线程归属"从约定变成了硬约束。
...
前九节的结论来自解析推导与数值模型，本节给出在真实 Paper 服务端上的实测：**让基准插件直接驱动生产类**，不重写任何算法逻辑，`SpiralCoordinateGenerator`、`RtpPool`、`SpatialMemory`、`RtpStorage` 跑的都是插件编译产物里的同一份字节码，列扫描通过反射调用 `RtpEngine` 的 public 方法 `findSafeSpotSync`。测试与复现脚本在仓库的 `benchmarks/` 目录下。

环境：Paper 26.2-124（API `26.2.build.124-stable`），Java 25.0.3，20 核，堆上限 2 GB，默认地形生成，世界边界 $6 \times 10^7$，无玩家在线。区块获取的代价与世界是否已预生成关系极大，因此这一项在**全新世界**与**已预生成世界**上各测一次。

同一构建共采集五轮。其中两类结果在五轮中逐位相同，可直接引用单轮值：流水线守恒的两组计数（132/120 与 14/14），以及 20000 次随机重读的 `hits=16618 misses=3383`。**两类是墙钟量，每轮不同**——坐标生成与 `SpatialMemory` 的写入及探针耗时——本节按五轮的区间给出，因为它反映的是单次测量而非稳定常数：同一台机器、同一份构建，探针耗时的最大值就是最小值的 2.4 倍。每一轮插件打印的摘要行保留在仓库的 `docs/bench/*/console.txt`。

有一条限制必须先说明：`/rtp` 命令本身需要已连接的客户端，无头运行无法提供，因此本节测量的是**引擎各组件在真实服务端上的行为**，而不是端到端命令延迟。第 10.4 节列出由此产生的偏差。

### 10.1 坐标生成

200,000 次 `spiral.next(world)` 耗时 36.8–40.2 ms（五轮区间），即 **497–543 万点/秒、184–201 ns/点**；20 万个点互不重复；单环配置的解析容量为 257708（式 9）。默认的三环配置在同一批点上为 44.1–48.7 ms（410–454 万点/秒），解析容量合计 641203——环数增加只让每次取环多一次 $O(k)$ 选择，测得的差别落在轮次波动之内。
...
两项离散度统计量上实测与模型的偏差都在 1% 以内（最近邻 $\sigma/\mu$ 为 $0.48\%$，面元计数为 $0.29\%$）。另外两行不能按同一标准要求：归一化面密度**极差**的模型值在未取整的理想坐标上取得，而实测值含整数取整，两者测的不是同一个对象；最小点间距则相差 $8\%$，因为 20000 点内的最小值是极值统计量，抽样方差本身就大——这一行要说的事是螺旋的 $13.93$ 格对随机的 $0.46$ 格，差距 $30$ 倍，而不是小数点后第二位。

**式 (8) 的面积密度恒定性因此在生产实现上可复现**：归一化面密度在 60 个径向分箱上的极差为 1.55%，且该残差来自有限样本与整数取整，不随 $N$ 累积。

顺带记一处只差一个括号的整数运算陷阱。容量估计要求 $\lfloor \pi(r_{\max}^2 - r_{\min}^2)/s^2 \rfloor$，若写成 `(long) Math.PI * (...)`，`(long) Math.PI` 会先算出 $3$ 并丢弃小数部分，容量估计随之降为 246093，比式 (9) 的 257708 低 4.5%。容量只用于索引取模，偏小不会让坐标算错，但螺旋会在环带最外约 96 格（面积的 4.5%）尚未被覆盖时提前回绕——这类错误不报错，只让覆盖范围悄悄缩小。取整必须在乘法之后，`RingZone.area()` 即写成 `(long) (Math.PI * (...))`。本节的测量不受这一处影响：一次采集只推进 220048 个索引（含暖机），小于 246093 与 257708 中的任何一个，螺旋从未走到回绕点。
...
- 写入 60000 个互不相同的单元耗时 37–62 ms（0.62–1.03 µs/单元，五轮），**内存缓存稳定在容量上限 50000**，LRU 边界在真实类上成立；
- 取一个**已被 LRU 淘汰**的单元（最先写入的那个），返回 `SAFE`，耗时 63.8 µs（五轮 63.8–152 µs）。此时该批写入中只有 15 个单元已落盘（`pendingCellCount = 59985`）。若没有第 4.3 节的"未落盘缓冲"层级，这次查询会落到数据库并返回 `UNKNOWN`。**该层级的存在由此被直接验证**，而不只是推导；
- 20000 次随机重读的命中率为 83.1%（五轮的 `hits` 与 `misses` 完全相同）。
...
同一批测量还给出了接受率：64 列中有 6 列存在满足露天约束的安全落点，$\hat{p} = 6/64 \approx 0.094$（表 1 用的就是这个值）。露天约束是刻意选择——玩家落点上方可见天空，不会一睁眼发现自己卡在洞里——接受率下降就是它的代价。

这里有一个容易忽略、但会直接改变这个数字含义的点：那 64 个候选列由固定的索引步长从同一条螺旋序列中选出，**每一轮采的都是同一批 64 列**（见 `benchmarks/`）。五轮中有四轮给出同一个 $6/64$，原因正在于此，而不是四轮独立地碰巧相等；把它们相加当成 $43/384$ 会得到一个并不存在的置信度。第五轮给出 $13/64$，同一批列得出不同结果只能来自世界状态本身，而该轮紧接在一次生成中途被中断的运行之后——也就是说，这一轮不能与其余各轮并列统计。要把样本量真正做大，必须让各轮采样不同的列，而不是重复测同一批。

还有一点同样重要：接受率是**世界与安全策略的性质**，不是插件的常数。在同一份构建、同一套默认策略下，一台长期运行的生存服务器的 RTP 摘要显示，220 秒内提交 146 个候选、其中 49 个通过校验，即 $0.34$——是基准世界那 64 列的三倍多，也落在它们给出的区间之外。表 1 因此应当按**敏感性表**来读：它说明的是尾部随 $p$ 减小而迅速变长，而不是断言某个 $p$ 值普遍成立。本文设计的依据是尾部的**存在性**（式 3 没有截断），这一点与 $p$ 取 $0.09$ 还是 $0.34$ 无关。
...
- **接受率只有 64 个互不相同的列，而且它随世界而变。** 二项标准误约 $\pm 0.036$，95% 置信区间约 $[0.02,\ 0.17]$，跨度很大；**重复采集不会增加有效样本量**，因为各轮的候选列是同一批（见 10.3 节）。同一套默认策略在一台生存服务器上给出 $0.34$，落在该区间之外，所以这个值只能作量级参考，不能当作插件的属性。加载时序同样来自 64 个区块，p99 的置信度有限。
- **单机、无跨环境重复。** 五轮都在同一台机器、同一份构建、同一种地形上进行，没有跨机器、跨地形类型或跨 JVM 的重复，也没有对墙钟量取多轮中位数，因此它们按区间给出而非单值。
...
表 8 汇总各环节的复杂度与资源上界。$k$ 为环数，$d$ 为 `maxScanDepth`。
...
需要说明的是，表 8 约束的是引擎自身的状态。区块加载后其区块所占内存由服务器的区块系统管理，本引擎不负责其释放。
...
四层验证覆盖的范围并不一致。第 10 节的实测直接驱动生产类，`SpiralCoordinateGenerator`、`RtpPool`、`SpatialMemory`、`RtpStorage` 与 `findSafeSpotSync` 都在其间，因此表 8 中"在途区块加载"与"池容量"两行有实测支持；其余各行，以及**端到端命令路径**——候选入池后的交付、`teleportAsync`、倒计时、Folia 上的区域调度——仍以代码审查与推导为依据。
...
所提出的设计把三者分离：选点化为闭式映射，面密度恒定为 $1/s^2$（式 8）；判定化为可复用、可持久化、可提前执行的纯函数；记忆化为有界状态，把"重试"替换为"跳过"。在此结构上引入分级预取（表 6）与并发背压（式 14），并把补齐度量修正为式 (13)，使预取成为需求驱动的稳态过程。

该结构的核心收益不是速度，而是**代价的可界定性**：表 8 中每一项都有明确上界，而朴素方案中没有任何一项具备这个性质。因此"提高重试上限"不构成解决方案，它的效果只是把失败换成卡顿。

第 10 节在真实 Paper 服务端上驱动生产类做了实测，其中三项结果改变或加强了本文的判断。其一，接受率实测约 $0.09$，显著低于按地形类别估计的 $0.35$，露天约束把拒绝采样的尾部拉长了约 550 倍（表 1）——这解释了现象的普遍性。其二，流水线守恒的失败在真实 `RtpPool` 上复现（五轮结果完全一致），且空闲服务器上的浪费量由校验速率而非目标容量决定，调小 `base-size` 无法缓解。其三，成本几乎全部落在区块获取上：判定（快照加列扫描）只占一次磁盘区块获取的 1.8%，而现场生成地形又比磁盘读取贵约 1158 倍。这三项合起来把设计的重心从"更快地判定"移到了"更早地知道该不该判定"。
